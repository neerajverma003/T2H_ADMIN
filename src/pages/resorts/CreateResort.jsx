                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                        <h1 className="text-3xl font-black text-slate-950 dark:text-white tracking-tight flex items-center gap-4">
                            <Building className="text-indigo-600" size={28} /> {id ? "EDIT RESORT" : "NEW RESORT"}
                        </h1>
                ...
                <div className={styleProps.cardStyle}>
                    ...
                    <div className="space-y-10">
                        <div className="w-full">
                            ...
                            <input name="title" value={formData.title} onChange={handleChange} className={styleProps.inputStyle} placeholder="e.g. Soneva Jani Luxury Villas" />
                        </div>
                        <div className="w-full">
                            <label className={styleProps.labelStyle}>Narrative Description</label>
                            <textarea name="description" value={formData.description} onChange={handleChange} className={`${styleProps.inputStyle} h-40 resize-none italic`} placeholder="Describe the romantic essence of this resort..." />
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div>
                                ...
                                <div className="relative group">
                                    ...
                                    <input type="number" name="price_per_night" value={formData.price_per_night} onChange={handleChange} className={`${styleProps.inputStyle} pl-12`} />
                                </div>
                            </div>
                            <div>
                                ...
                                <div className="relative group">
                                    ...
                                    <input type="number" name="duration_days" value={formData.duration_days} onChange={handleChange} className={`${styleProps.inputStyle} pl-14`} />
                    ...
                    <div className="space-y-12">
                        <label className="group relative flex flex-col items-center justify-center w-full aspect-[21/7] border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-3xl cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all hover:border-indigo-600">
                            ...
                            <div className="text-center">
                                <p className="text-sm font-black uppercase tracking-[0.2em] text-slate-950 dark:text-white mb-0.5">Acquire High-Res Assets</p>
                                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest italic opacity-70">UHD/4K Processing Pipeline Optimized</p>
                            </div>
                        ...
                        {(previewUrls.length > 0 || existingImages.length > 0) && (
                            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-6 bg-slate-50 dark:bg-slate-800/30 p-8 rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-inner">
                                {existingImages.map((src, idx) => (
                                    <div key={idx} className={`relative aspect-square rounded-[2rem] overflow-hidden group border-4 ${removedImageIndexes.includes(idx) ? 'border-red-600 opacity-50' : 'border-transparent shadow-xl'}`}>
                                        ...
                                        <button type="button" onClick={() => setRemovedImageIndexes(p => p.includes(idx) ? p.filter(i => i !== idx) : [...p, idx])} className="absolute inset-0 bg-slate-950/60 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-all backdrop-blur-[2px]">
                ...
                <div className="space-y-10">
                    <div className={styleProps.cardStyle}>
                        <div className="flex items-center gap-4 mb-10">
                            ...
                            <h2 className="text-lg font-black text-slate-950 dark:text-white uppercase tracking-tight">Portal Visibility Settings</h2>
                        </div>
                        <div className="space-y-6">
                            <div className="w-full">
                                <label className={styleProps.labelStyle}>Visibility Matrix</label>
                                <select name="visibility" value={formData.visibility} onChange={handleChange} className={styleProps.inputStyle}>
                                    <option value="public">Published on Public Portal</option>
                                    <option value="private">Hidden / Internal Draft</option>
                                </select>
                            </div>
                            <div className="w-full">
                                <label className={styleProps.labelStyle}>Availability Status Logic</label>
                                <select name="availability_status" value={formData.availability_status} onChange={handleChange} className={styleProps.inputStyle}>
                                    <option value="Available">Available for Direct Booking</option>
                                    <option value="Unavailable">Maintenance / Offline Mode</option>
                                    <option value="Booked">Fully Committed / Sold Out</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className={styleProps.cardStyle}>
                        <div className="flex items-center gap-4 mb-10">
                            ...
                            <h2 className="text-lg font-black text-slate-950 dark:text-white uppercase tracking-tight">Market Positioning Status</h2>
                        </div>
                        <div className="space-y-6">
                            <div className="flex items-center justify-between p-6 bg-slate-50 dark:bg-slate-800/50 rounded-3xl border-2 border-slate-100 dark:border-slate-800 shadow-inner">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <p className="text-base font-black text-slate-900 dark:text-white uppercase tracking-tight">Global Activation</p>
                                        <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest italic opacity-60">Visible across all channels</p>
                                    </div>
                                </div>
                                <input type="checkbox" name="is_active" checked={formData.is_active} onChange={handleChange} className="size-8 accent-emerald-600 cursor-pointer" />
                            </div>
                            <div className="flex items-center justify-between p-6 bg-slate-50 dark:bg-slate-800/50 rounded-3xl border-2 border-slate-100 dark:border-slate-800 shadow-inner">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <p className="text-base font-black text-slate-900 dark:text-white uppercase tracking-tight">Featured Premium</p>
                                        <p className="text-[10px] font-black text-amber-600 uppercase tracking-widest italic opacity-60">Prioritized in engines</p>
                                    </div>
                                </div>
                                <input type="checkbox" name="is_featured" checked={formData.is_featured} onChange={handleChange} className="size-8 accent-amber-500 cursor-pointer" />
                            </div>
                        </div>
                    </div>
                </div>
