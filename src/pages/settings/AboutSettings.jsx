        ...
        <form onSubmit={handleSaveStory} className="space-y-12">
          ...
          <div className={styleProps.cardStyle}>
            <h3 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tight mb-8">
              A. Hero Banner Layout
            </h3>
            <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
              <div>
                <label className={styleProps.labelStyle}>Cinematic Media URL (Image or S3 Video)</label>
                <div className="relative group">
                  <FiSettings className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                  <input
                    type="text"
                    value={storyForm.hero.mediaUrl}
                    onChange={(e) => handleNestedChange("hero", "mediaUrl", e.target.value)}
                    placeholder="e.g., about-hero-video.mp4 or absolute image link"
                    className={styleProps.inputStyle}
                    required
                  />
                </div>
                
                {/* Drag & Drop Hero Media zone */}
                <div className="mt-4">
                  <label 
                    className="flex flex-col items-center justify-center gap-3 cursor-pointer rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/20 p-6 hover:border-indigo-500 transition-all text-center relative overflow-hidden group"
                    onDragOver={(e) => e.preventDefault()}
                    onDrop={handleHeroDrop}
                  >
                    {uploadingHero ? (
                      <div className="flex flex-col items-center gap-2">
                        <Loader2 className="animate-spin text-indigo-700 size-8" />
                        <p className="text-xs font-bold text-slate-400">Uploading cinematic media to S3...</p>
                      </div>
                    ) : (
                      <>
                        <UploadCloud className="text-slate-400 size-8 group-hover:text-indigo-600 transition-colors" />
                        <div className="text-xs text-slate-600 dark:text-slate-400">
                          <span className="text-indigo-700 font-bold">Click to upload</span> or drag and drop
                          <p className="text-[10px] text-slate-400 mt-1 uppercase tracking-widest">Supports Images & cinematic Videos</p>
                        </div>
                        <input type="file" accept="image/*,video/*" onChange={handleHeroFileSelect} className="hidden" />
                      </>
                    )}
                  </label>
                </div>
              ...
              <div>
                <label className={styleProps.labelStyle}>Hero Banner Tagline</label>
                <div className="relative group">
                  <FiSettings className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                  <input
                    type="text"
                    value={storyForm.hero.tagline}
                    onChange={(e) => handleNestedChange("hero", "tagline", e.target.value)}
                    placeholder="Embark On The Journey Of A Lifetime"
                    className={styleProps.inputStyle}
                  />
                </div>
              </div>
              <div className="md:col-span-2">
                <label className={styleProps.labelStyle}>Main Hero Massive Title</label>
                <div className="relative group">
                  <FiSettings className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                  <input
                    type="text"
                    value={storyForm.hero.title}
                    onChange={(e) => handleNestedChange("hero", "title", e.target.value)}
                    placeholder="Where Love Begins \n Memories Last Forever"
                    className={styleProps.inputStyle}
                  />
                </div>
              </div>
              <div className="md:col-span-2">
                <label className={styleProps.labelStyle}>Hero Subtitle / Mini Description</label>
                <div className="relative group">
                  <FiSettings className="absolute left-4 top-5 text-slate-400" size={18} />
                  <textarea
                    value={storyForm.hero.subtitle}
                    onChange={(e) => handleNestedChange("hero", "subtitle", e.target.value)}
                    placeholder="Brief intro block text..."
                    rows="3"
                    className={`${styleProps.inputStyle} h-28 pt-4 resize-none`}
                  />
                </div>
              </div>
            </div>
