...
const ActivityEnquiries = () => {
  ...
  const loadEnquiries = async () => {
    try {
      setIsLoading(true);
      setError(null);
      const res = await apiClient.get("/admin/activity-enquiries");
      if (res.data.Data) {
        setEnquiries(res.data.Data);
      }
    } catch (err) {
      console.error("Error loading activity enquiries:", err);
      setError("Failed to load activity enquiries.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Permanently delete this activity enquiry?")) return;
    try {
      const response = await apiClient.delete(`/admin/activity-enquiries/${id}`);
      if (response.data.success) {
        setEnquiries((current) => current.filter((e) => e._id !== id));
        toast.success("Activity enquiry deleted successfully");
  ...
  const handleStatusChange = async (enquiryId, newStatus) => {
    ...
    try {
      const response = await apiClient.put(
        `/admin/activity-enquiries/${enquiryId}/status`,
        { status: newStatus }
      );
      if (response.data.success) {
        ...
        <div className="relative z-10 flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="text-2xl md:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight flex items-center gap-2 flex-wrap">
              Activity <span className="text-blue-500">Enquiries</span>
            </h1>
            <p className="text-slate-500 dark:text-slate-400 font-semibold mt-0.5 text-xs sm:text-sm">
              Manage custom enquiries and couple booking requests submitted for curated activities.
            </p>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="py-28 text-center bg-slate-50 dark:bg-slate-950/60 rounded-3xl border border-slate-200 dark:border-slate-800 border-dashed shadow-sm">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              {searchTerm ? "No results match your search criteria." : "Enquiries submitted on activity pages will appear here."}
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-950/80 border-b border-slate-200 dark:border-slate-800/80 text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-wider">
                  ...
                  <th className="p-4">ACTIVITY & DESTINATION</th>
                    ...
                    >
                      ...
                      <td className="p-4 max-w-xs">
                        <div className="flex flex-col gap-1">
                          <span className="font-extrabold text-xs text-slate-900 dark:text-white line-clamp-1">
                            {item.activityTitle || "Curated Activity"}
                          </span>
                      ...
                      <td className="p-4">
                        <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">
                          {item.travelDate || "Flexible / Not specified"}
                        </span>
                      </td>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md">
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 space-y-1">
                  <span className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                    <Ticket size={12} className="text-blue-600 dark:text-blue-400" /> ACTIVITY
                  </span>
                  <p className="text-xs font-bold text-slate-900 dark:text-white">
                    {selectedEnquiry.activityTitle || "Curated Activity"}
                  </p>
                </div>

                <div className="bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 space-y-1">
                  <span className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                    <MapPin size={12} className="text-blue-600 dark:text-blue-400" /> DESTINATION
                  </span>
                  <p className="text-xs font-bold text-slate-900 dark:text-white uppercase">
                    {selectedEnquiry.destination || "General"}
                  </p>
                </div>

                <div className="bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 space-y-1">
                  <span className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                    <Calendar size={12} className="text-blue-600 dark:text-blue-400" /> TARGET TRAVEL DATE
                  </span>
                  <p className="text-xs font-bold text-slate-900 dark:text-white">
                    {selectedEnquiry.travelDate || "Flexible / Not specified"}
                  </p>
                </div>

                <div className="bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 space-y-1">
                  <span className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                    <Clock size={12} className="text-blue-600 dark:text-blue-400" /> SUBMITTED DATE
                  </span>
                  <p className="text-xs font-bold text-slate-900 dark:text-white font-mono">
              ...
              {selectedEnquiry.message && (
                <div className="bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 space-y-1.5">
                  <span className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                    <MessageSquare size={12} className="text-blue-600 dark:text-blue-400" /> CUSTOMER NOTES / MESSAGE
                  </span>
...
export default ActivityEnquiries;
