  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-full mx-auto space-y-10 pb-20 px-6 text-left">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[3rem] p-12 border border-slate-100 dark:border-slate-800 shadow-2xl shadow-slate-200/50 dark:shadow-none relative overflow-hidden">
        <div className="absolute top-0 right-0 p-12 opacity-5 pointer-events-none text-indigo-700"><FileText size={240} /></div>
        <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-10">
            <div>
                <h1 className="text-4xl font-black text-slate-950 dark:text-white tracking-tight flex items-center gap-5">
                    <ShieldCheck className="text-indigo-700" size={44} /> COMPLIANCE REGISTRY
                </h1>
                <p className="text-slate-600 dark:text-slate-400 font-bold mt-2 text-xl italic text-left">Global terms & conditions for destination engagement</p>
            </div>
            <div className="flex items-center gap-5">
                <div className="px-8 py-5 bg-indigo-700 text-white rounded-[1.5rem] text-sm font-black uppercase tracking-[0.2em] flex items-center gap-3 shadow-2xl shadow-indigo-500/40">
                    <Sparkles size={20} /> GOVERNANCE SECURED
                </div>
            </div>
        </div>
      </div>
                    ...
                    >
                        <div className="flex items-center gap-6">
                            <div className={`size-16 rounded-[1.25rem] flex items-center justify-center transition-all shadow-xl ${
                                ...
                                <tab.icon size={32} strokeWidth={2.5} />
                            </div>
                            <span className="text-xl font-black uppercase tracking-[0.15em]">{tab.label}</span>
                        </div>
                        <Zap size={24} className={activeTab === tab.id ? "opacity-100 animate-pulse" : "opacity-0 group-hover:opacity-100 transition-opacity"} />
                    </button>
                ))}
            </div>
            <div className="mt-8 pt-8 border-t-4 border-slate-50 dark:border-slate-800">
                <div className="flex items-center gap-6 p-8 rounded-[2.5rem] bg-indigo-50 dark:bg-indigo-900/10 border-4 border-indigo-100 dark:border-indigo-900/20 shadow-inner">
                    <CheckCircle2 size={32} className="text-indigo-700 shrink-0" />
                    <p className="text-sm font-black text-indigo-800 dark:text-indigo-400 uppercase tracking-widest leading-relaxed">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8 mb-12 pb-12 border-b-4 border-slate-50 dark:border-slate-800">
                <div className="flex items-center gap-6">
                    ...
                    <div className="text-left">
                       <h2 className="text-3xl font-black text-slate-950 dark:text-white tracking-tight uppercase">{activeTab} Framework</h2>
                       <p className="text-xs font-black text-indigo-700 uppercase tracking-widest mt-2">Authorized Compliance Context Engine</p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-5 w-full md:w-auto">
                    {!isEditing ? (
                        <button
                            ...
                            disabled={loading}
                            className="w-full md:w-auto px-12 py-6 bg-slate-950 text-white rounded-[1.5rem] text-sm font-black uppercase tracking-[0.2em] hover:bg-slate-800 transition-all shadow-2xl shadow-black/20 disabled:opacity-30 flex items-center justify-center gap-4"
                        >
                            <Edit size={22} /> Edit Compliance Registry
                        </button>
                    ) : (
                        <button
                            onClick={handleSave}
            ...
            <div className="flex-1 flex flex-col">
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-48 gap-8">
                        ...
                        <p className="text-xs font-black uppercase tracking-[0.4em] text-slate-400">Loading Registry Assets...</p>
                    </div>
                ) : isEditing ? (
                    <textarea
                        value={textContent}
                    ...
                    <div className="flex-1 p-12 text-slate-950 dark:text-slate-300 text-2xl font-black leading-relaxed whitespace-pre-line bg-slate-50 dark:bg-slate-800/30 rounded-[3rem] border-4 border-slate-100 dark:border-slate-800/50 shadow-inner text-left italic min-h-[500px]">
                        {textContent || "Compliance registry is currently empty and pending documentation."}
                    </div>
                )}
            </div>
        </div>
