...
import { Star, User, MessageSquare, Trash2, Plus, Sparkles, Image as ImageIcon } from "lucide-react";
        ...
        <div className={styles.cardStyle}>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Traveler Stories</h2>
                        <p className="text-slate-500 dark:text-slate-400 text-sm font-bold uppercase tracking-widest mt-1 flex items-center gap-2">
                            <Sparkles size={14} className="text-indigo-500" /> Dynamic Itinerary Reviews
                        </p>
                    </div>
                </div>
                <button
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {(formData.reviews || []).map((review, index) => (
                    <div key={index} className="relative group bg-slate-50/50 dark:bg-slate-800/30 rounded-[2rem] p-8 border border-slate-100 dark:border-slate-800 hover:border-indigo-200 dark:hover:border-indigo-500/30 transition-all duration-500">
                        <button
                            type="button"
                        ...
                        <div className="flex items-start gap-6">
                            ...
                            <div className="flex-shrink-0">
                                <label className="relative size-24 rounded-[2rem] bg-white dark:bg-slate-900 border-2 border-dashed border-slate-200 dark:border-slate-700 overflow-hidden cursor-pointer hover:border-indigo-500 transition-all group/p shadow-inner">
                                    <input
                                        type="file"
                                        accept="image/*"
                                        className="hidden"
                                    ...
                                    <div className="flex gap-1 bg-white dark:bg-slate-900 p-2 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm">
                                        {[1, 2, 3, 4, 5].map((star) => (
                                            <button
                                                ...
                                                className={`transition-all ${star <= review.rating ? "text-amber-400 scale-110" : "text-slate-200 dark:text-slate-700 hover:text-amber-200"}`}
                                            >
                        ...
                        <div className="mt-6">
                            ...
                            <textarea
                                ...
                                onChange={(e) => handleArrayChange(e, index, "reviews")}
                                className={`${styles.inputStyle} h-32 resize-none`}
            ...
            {(!formData.reviews || formData.reviews.length === 0) && (
                <div className="text-center py-20 bg-slate-50/50 dark:bg-slate-800/30 rounded-[3rem] border-2 border-dashed border-slate-200 dark:border-slate-800">
                    ...
                    <h3 className="text-xl font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest">No Traveler Stories Yet</h3>
                    <p className="text-slate-500 mt-2 font-medium">Click "Add New Story" to showcase traveler experiences for this trip.</p>
                </div>
            )}
        </div>
    );
