...
const ReferralAudit = () => {
  ...
  return (
    ...
    >
      {/* HEADER SECTION */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-gradient-to-r from-indigo-500/10 via-purple-500/10 to-indigo-600/10 dark:from-slate-900 dark:to-indigo-950 bg-white dark:bg-transparent p-8 rounded-[2rem] border border-indigo-200 dark:border-indigo-900/30 shadow-xl dark:shadow-2xl relative overflow-hidden">
        <div className="absolute top-0 right-0 w-80 h-80 bg-indigo-500/10 rounded-full blur-3xl pointer-events-none" />
        <div className="relative z-10">
          <div className="flex items-center gap-3">
            <span className="p-2.5 bg-indigo-100 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 rounded-xl">
              <Shield size={24} />
            </span>
            <span className="text-xs font-extrabold uppercase tracking-[0.2em] text-indigo-600 dark:text-indigo-400">Security Control</span>
          </div>
          <h1 className="text-3xl lg:text-4xl font-black text-slate-900 dark:text-white tracking-tight mt-3">
            Referral & Wallet Audit
          </h1>
          <p className="text-slate-600 dark:text-slate-400 font-medium mt-2 text-sm max-w-xl">
            Monitor registration velocities, trace promotional reward allocations, audit automated logs, and manage manual lock states.
          </p>
        </div>

        {/* Tab Switcher */}
        <div className="flex bg-slate-100 dark:bg-slate-800/80 p-1.5 rounded-2xl border border-slate-200 dark:border-slate-700/50 self-start lg:self-center relative z-10">
          <button
      ...
      {/* SEARCH AND FILTERS */}
      {activeTab === "customers" && (
        <div className="flex items-center justify-between gap-4 bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800/60 shadow-md">
          <div className="relative w-full max-w-md">
            ...
            <input
              ...
              className="pl-11 pr-6 py-3 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-transparent focus:border-indigo-500 rounded-xl text-sm font-semibold text-slate-900 dark:text-white placeholder:text-slate-400 focus:ring-2 focus:ring-indigo-500/10 w-full transition-all"
            />
          </div>
      ...
      {/* CONTENT TABLES */}
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800/50 shadow-xl overflow-hidden min-h-[400px] relative">
        {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800">
                  <tr>
                    <th className="px-8 py-5 text-left font-black text-slate-600 dark:text-slate-400 uppercase tracking-wider text-xs">Customer</th>
                    <th className="px-6 py-5 text-left font-black text-slate-600 dark:text-slate-400 uppercase tracking-wider text-xs">Referral Code</th>
                    <th className="px-6 py-5 text-left font-black text-slate-600 dark:text-slate-400 uppercase tracking-wider text-xs">Wallet Balance</th>
                    <th className="px-6 py-5 text-center font-black text-slate-600 dark:text-slate-400 uppercase tracking-wider text-xs">Referrals Made</th>
                    <th className="px-6 py-5 text-center font-black text-slate-600 dark:text-slate-400 uppercase tracking-wider text-xs">Wallet State</th>
                    <th className="px-8 py-5 text-right font-black text-slate-600 dark:text-slate-400 uppercase tracking-wider text-xs">Action</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                  {filteredCustomers.map((user) => (
                    <tr key={user._id} className="group hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-colors">
                      ...
                      <td className="px-6 py-5">
                        <span className="px-3 py-1 bg-slate-100 dark:bg-slate-800/80 rounded-lg text-slate-800 dark:text-slate-300 font-mono text-xs font-bold border border-slate-200 dark:border-slate-700/40">
                      ...
                      <td className="px-6 py-5 text-center">
                        {user.is_wallet_frozen ? (
                          <span className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-sky-50 dark:bg-sky-950/40 text-sky-700 dark:text-sky-400 text-xs font-extrabold border border-sky-200 dark:border-sky-900/30">
                            <Snowflake size={12} className="animate-pulse" />
                            Frozen
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-400 text-xs font-extrabold border border-emerald-200 dark:border-emerald-900/30">
                            <Sun size={12} />
                            Active
                          </span>
                        )}
                      ...
                      <td className="px-8 py-5 text-right">
                        <button
                          ...
                          className={`px-4 py-2 rounded-xl text-xs font-extrabold tracking-wider uppercase transition-all shadow-xs border ${
                            user.is_wallet_frozen
                              ? "bg-emerald-600 hover:bg-emerald-700 text-white border-emerald-500 shadow-emerald-500/10"
                              : "bg-red-50 dark:bg-red-950/20 text-red-600 dark:text-red-400 border-red-200 dark:border-red-900/30 hover:bg-red-600 hover:text-white"
                          }`}
                        >
          ...
          logs.length > 0 ? (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800">
                  <tr>
                    <th className="px-8 py-5 text-left font-black text-slate-600 dark:text-slate-400 uppercase tracking-wider text-xs">Referrer Account</th>
                    <th className="px-6 py-5 text-left font-black text-slate-600 dark:text-slate-400 uppercase tracking-wider text-xs">Security Event</th>
                    <th className="px-6 py-5 text-left font-black text-slate-600 dark:text-slate-400 uppercase tracking-wider text-xs">Violation Details</th>
                    <th className="px-6 py-5 text-center font-black text-slate-600 dark:text-slate-400 uppercase tracking-wider text-xs">Referrals (24h)</th>
                    <th className="px-8 py-5 text-right font-black text-slate-600 dark:text-slate-400 uppercase tracking-wider text-xs">Timestamp</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                  {logs.map((log) => (
                      ...
                      <td className="px-8 py-5">
                        {log.referrerId ? (
                          <div>
                            ...
                            <p className="text-xs text-slate-500 dark:text-slate-400 font-semibold">{log.referrerId.email}</p>
                          </div>
                        ) : (
        ...
        {isModalOpen && selectedUser && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-slate-800/80 max-w-lg w-full overflow-hidden shadow-2xl relative z-10 p-8"
            >
              ...
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 focus:border-indigo-500 rounded-xl p-4 text-sm font-semibold text-slate-900 dark:text-white focus:ring-4 focus:ring-indigo-500/10 transition-all outline-none resize-none"
                  />
                </div>
              </div>
              ...
              <div className="flex items-center justify-end gap-3 mt-8">
                <button
                  onClick={() => setIsModalOpen(false)}
                  className="px-5 py-3 rounded-xl text-xs font-extrabold uppercase tracking-wider text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 transition-all"
                >
                  Cancel
                </button>
                <button
